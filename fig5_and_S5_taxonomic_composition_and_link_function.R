...
packages <- c("stringr", "reshape2","digest","AnnotationDbi", "ggplot2", 'scales',"GO.db", "spaa", "xlsx","FactoMineR", "factoextra", "xlsx", "dplyr", "zoo", "ggplot2", "gplots", "reshape2", "RColorBrewer", 
...
otu_depth_sub=otu_depth_sub[,-which(names(otu_depth_sub) %in% c("job_id","sequence_number", 
...
colors_own=unname(c(piratepal("basel"), piratepal("pony"), piratepal("eternal"), 
...
for (i in 1:length(dbs)){ 
  ...
  tax_function=tpm_tax3[,which (names (tpm_tax3) %in% c("domain","kingdom","phylum","class","order","family","genus","species",
  ...
  tax_abund=tpm_tax3[,which (names (tpm_tax3) %in% c("domain","kingdom","phylum","class","order","family","genus","species",
                                                     paste(metadata$id)))]
  
  
  tax_abund2=tpm_tax3[,which (names (tpm_tax3) %in% c("phylum",
                                                      paste(metadata$id)))]
