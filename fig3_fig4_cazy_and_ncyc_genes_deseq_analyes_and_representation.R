...
ncyc_lvls=c("Nitrogen fixation","Nitrification","Denitrification","Dissimilatory nitrate reduction","Denitrification, Dissimilatory nitrate reduction","Anammox","Assimilatory nitrate reduction","Organic degradation and synthesis","Others")
...
theme_rep_1= theme(panel.background= element_blank(),
                   panel.grid.major = element_blank(),
                   panel.grid.minor = element_blank(),
                   strip.background =element_rect(fill="white"),
...
#selection of genes to represent and representation of CAZYs fig 3
...
for (i in 1:length(dbs)) {
  for (k in 1:length(names_tables_ds)) {
    ...
    dds_taxa_summary_up=summarySE(dds_taxa_subset_up, measurevar="log2FoldChange", groupvars=c("class2", "class3", "trend"))
    
    print(setdiff(dds_taxa_summary_up$class3, table$class3))
    
    dds_taxa_summary_up=merge(dds_taxa_summary_up, table, by="class3", all=F)
    dds_taxa_summary_up$count2=counts2$rel[match(dds_taxa_summary_up$class3, counts2$class3)]
    ...
    dds_taxa_summary_down=summarySE(dds_taxa_subset_down, measurevar="log2FoldChange", groupvars=c("class2", "class3", "trend"))
    
    print(setdiff(dds_taxa_summary_down$class3, table$class3))
    
    dds_taxa_summary_down=merge(dds_taxa_summary_down, table, by="class3", all=F)
    dds_taxa_summary_down$count2=counts2$rel[match(dds_taxa_summary_down$class3, counts2$class3)]
    
    
    dds_taxa_summary=rbind(dds_taxa_summary_up, dds_taxa_summary_down)
    dds_taxa_summary[is.na(dds_taxa_summary)] <- 0
    dds_taxa_summary$class_desc=as.factor(paste(dds_taxa_summary$activity_main,"(",dds_taxa_summary$class2,")", sep=""))
    
    
    if (i==1) {
      dds_taxa_summary=dds_taxa_summary[(dds_taxa_summary$compound_associated %in% lvls),]  
      ...
      dds_taxa_summary=dds_taxa_summary[order(factor(dds_taxa_summary$phylum, levels = lvls), -dds_taxa_summary$log2FoldChange),]
      dds_taxa_summary$compound_associated=dds_taxa_summary$phylum
      
      }
    
    order=dds_taxa_summary$class2
    ...
    #represent like bars
      ...
      rep=ggplot(dds_taxa_summary, aes(factor(class2, levels = rev(order)), log2FoldChange)) + 
        ...
        geom_point(aes(size = count2, y=max(na.pass(dds_taxa_summary$log2FoldChange + dds_taxa_summary$sd))+1), color="black", show.legend=TRUE, pch=21, fill="darkgrey",alpha=0.5) +
        scale_size(breaks = seq(10^-6, 10^-4, by=2*10^-5))+
        facet_grid(factor(compound_associated, levels=lvls)~. , scales = "free", space = "free")+
        theme(panel.spacing = unit(1, "lines"))+
        coord_flip(clip = "on")+
        xlab("") + 
        ggtitle(paste(names_tables_ds[k], dbs[i]), "class2") +
      ...
      if (i==2) {
        ...
        dds_taxa_summary2=rbind(dds_taxa_subset_up,dds_taxa_subset_down)
        dds_taxa_summary2$phylum=table$phylum[match(dds_taxa_summary2$class3,table$class3)]
        dds_taxa_summary2$class4=table$class4[match(dds_taxa_summary2$class3,table$class3)]
        
        dds_taxa_summary3=summarySE(dds_taxa_summary2, measurevar="log2FoldChange", groupvars=c("phylum"))
        dds_taxa_summary3$soils=rep(pair[k],nrow(dds_taxa_summary3))
        
        dds_taxa_summary4=summarySE(dds_taxa_summary2, measurevar="log2FoldChange", groupvars=c("phylum","class4"))
        dds_taxa_summary4$soils=rep(pair[k],nrow(dds_taxa_summary4))
    ...
    assign(paste(names_tables_ds[k],dbs[i], "dds_taxa_summary3",sep="_"), dds_taxa_summary3)
    assign(paste(names_tables_ds[k],dbs[i], "dds_taxa_summary4",sep="_"), dds_taxa_summary4)
    
    
    assign(paste(dbs[i], "all_pairs", sep="_"), all_pairs)
...
#represent nitrogen simplified fig 4
...
ncyc_all4=rbind(names2_b_N160_vs_a_N10_ncyc_dds_taxa_summary4,names2_b_N160_vs_a_S160_ncyc_dds_taxa_summary4,names2_b_N10_vs_a_S10_ncyc_dds_taxa_summary4)
ncyc_all4=ncyc_all4[order(factor(ncyc_all4$phylum, levels = ncyc_lvls), factor(ncyc_all4$soils, levels=pair),ncyc_all4$class4),]
ncyc_all4$trend=unlist(lapply(ncyc_all4$log2FoldChange, fill_trend))
ncyc_all4=ncyc_all4[ncyc_all4$phylum!="Others",]

for (i in 1:length(pair[1:3])) {
  print(pair[i])
  table=ncyc_all4[ncyc_all4$soils==pair[i],]
  
  rep=ggplot(table, aes(class4, log2FoldChange, fill=trend)) +
    ...
    xlab("")+
    coord_flip()+
    facet_grid(rows = vars(factor(table$phylum, levels = ncyc_lvls)),scales="free", space="free")+
...
#########################   DIVERSITY AND ABUNDANCE, fig S3 and S4 ###############################
